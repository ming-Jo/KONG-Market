...
const USER_NAME = getStorageItem === null ? null : JSON.parse(getStorageItem).username;
...
export const fetchLogin = createAsyncThunk(
  ...
  async ({ username, password, login_type }: LoginData, { rejectWithValue }) => {
    try {
      ...
      if (response.data) {
        sessionStorage.setItem('token', JSON.stringify({ username, ...response.data }));
      }
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchLogin.rejected, (state, action) => {
      ...
      if (action.payload) {
        ...
        state.error = action.error.message || '로그인에 실패하였습니다. 다시 시도해주세요.';
      }
    });
    // 로그아웃 시 실행될 리듀서
